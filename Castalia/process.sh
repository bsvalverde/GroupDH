...
for SCA in ../results/*.sca; do
    ...
    echo "$SCA -> ./$DATA"
    echo "TXedPkts,DeliveryRatio,FairnessIndex,energyConsumed,ETEDelayMin,ETEDelayMean,ETEDelayMax,HashCollisions,HashCollisionsTotal,KeepAlives,ClockDrift" > $DATA
    ...
    CLOCKDRIFT=$(grep "\"Average clock error (ts)\"" "$SCA" | head -1 | awk '{print $7}')
